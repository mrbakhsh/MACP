  ...
  #' This function is adapted from the \code{\link[HPiP]{pred_ensembel}}
  #'  function in the HPiP package.
  #' @export
  #' @examples
  #' library(tidyr)
  #' library(dplyr)
  ...
  #' exampleData <- data_filtering(exampleData)
  #' # scoring
  #' ppi <- calculate_PPIscore(exampleData,
  #'                         PCCN = FALSE)
  #'# separate the interaction pairs
  #'PPI_pairs <-
  #'  ppi %>%
  #'  separate(PPI, c("p1", "p2"), sep = "~") %>% select(p1,p2)
  #' #load reference complexes
  #' data("refcpx")
  #' # generate ref_int
  #' class_labels <- generate_refInt(PPI_pairs, refcpx)
  #' # Model prediction
  #' predInt <- ensemble_model(ppi,class_labels)
  #' # Extract predicted interactions
  #' predInt <- predInt[["predicted_interactions"]]
